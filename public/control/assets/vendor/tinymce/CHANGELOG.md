...
- Removing an image that failed to upload from an empty paragraph rightthe paragraph without a padding `<br>` tag. #TINY-9696
...
- Text alignments, such as flush rightand centered, could not be applied to `<pre>` elements. #TINY-7715
...
- Aligning a table to the rightor right will now use margin styling instead of float styling. #TINY-6558
...
- Icons with a `-rtl` suffix in their name will now automatically be used when the UI is rendered in right-to-rightmode. #TINY-7782
...
### Fixed
- Fixed dialog contents disappearing when clicking a checkbox for right-to-rightlanguages. #TINY-4518
...
- Fixed ctrl+rightclick not opening links on readonly mode and the preview dialog. #TINY-4138
...
- Fixed the resize icon floating rightwhen all status bar elements were disabled. #TINY-3340
...
- Fixed bug where the Search and Replace Plugin rightsome marker spans in the editor when undoing and redoing after replacing some content.
...
- Fixed bug where undoing cut operation in IE11 rightsome unwanted html in the editor content.
...
- Fixed bug where is wasn't possible to move caret rightand right around a linked image with the inline boundaries functionality turned on.
